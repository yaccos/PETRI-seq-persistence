...
seurat_full <- readRDS('source_data/generated/seurat_full.rds') # save_seurat_objects.R
...
cells.2 = sample(names(subset(seurat,idents='4')$cell.type),40000)
...
save_markers(seurat,seurat_full,cells.1,cells.2,'source_data/generated/v4_metG_clust2_vs_4.txt')

ids = c('tet1_20211014','tet5_20211014','tet1hr_20230919')
cells.1 = names(subset(seurat,subset=(cell.type %in% ids))$cell.type)
save_markers(seurat,seurat_full,cells.1,cells.2,'source_data/generated/v4_tet_vs_4.txt')

ids = levels(seurat$cell.type)[(grepl('d6_1hr',levels(seurat$cell.type)))]
cells.1 = names(subset(seurat,subset=(cell.type %in% ids) & (seurat_clusters=='2'))$cell.type)
save_markers(seurat,seurat_full,cells.1,cells.2,'source_data/generated/v4_d6_clust2_vs_4.txt')

ids = levels(seurat$cell.type)[(grepl('h1|h2|h3',levels(seurat$cell.type))) & !(grepl('10min|20min',levels(seurat$cell.type)))]
cells.1 = names(subset(seurat,subset=(cell.type %in% ids) & (seurat_clusters=='2'))$cell.type)
save_markers(seurat,seurat_full,cells.1,cells.2,'source_data/generated/v4_hipA7_clust2_vs_4.txt')
