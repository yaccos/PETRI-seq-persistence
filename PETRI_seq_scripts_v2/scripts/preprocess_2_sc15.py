...
def log_to_table(sample, bc_round):
	...
	if bc_round != 'bc1':
		...
		exit()
	log = open(f'{sample}_logs/sc_pipeline_15/{bc_round}.log')
	table = open(f'{sample}_{bc_round}_table.txt', 'w')
	table.write(f'{bc_round} num\tcount\n')
	next_line = False
	for line in log:
		if 'Command line parameters' in line:
			bcs = line[line.find(f'{sample}_{bc_round}/') + len(sample) + len(bc_round) + 2:line.find('{name}')]
...
def log_plot(sample, bc_round, cell_num):
	...
	if cell_num > 0:
		if os.path.exists(f'{sample}_{bc_round}_cumulative_frequency_table.txt'):
			table = pd.read_csv(f'{sample}_{bc_round}_cumulative_frequency_table.txt', sep='\t')
			threshold_table = table.loc[table['index'] == int(cell_num)]
			fig.axvline(np.log10(int(threshold_table['count'])), color='k', linestyle='--')
	fig.set_title(f'{sample} {bc_round}, total reads = {sum(table["count"])}')
	fig.set_xlabel('log10(number of reads)')
	fig.set_ylabel('frequency')
	fig.get_figure().savefig(f'{sample}_{bc_round}_ReadsPerBC.eps')
	plt.cla()
...
def freq_plot(sample, bc_round, cell_num):
	if os.path.exists(f'{sample}_{bc_round}_cumulative_frequency_table.txt'):
		...
		table.to_csv(f"{sample}_{bc_round}_cumulative_frequency_table.txt", sep='\t')
	...
	plt.tight_layout()
	fig.savefig(f"{sample}_{bc_round}_kneePlot.eps")
