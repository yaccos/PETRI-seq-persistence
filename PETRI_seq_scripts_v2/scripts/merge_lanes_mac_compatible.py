...
for prefix in prefixes:
  ...
  print("gunzip -c %s | gzip > %s_R1_all_lanes.fastq.gz" % (file_str_1,prefix))
  print("gunzip -c %s | gzip > %s_R2_all_lanes.fastq.gz" % (file_str_2,prefix))
  subprocess.call("gunzip -c %s | gzip > %s_R1_all_lanes.fastq.gz" % (file_str_1,prefix), shell=True)
  subprocess.call("gunzip -c %s | gzip > %s_R2_all_lanes.fastq.gz" % (file_str_2,prefix), shell=True)
