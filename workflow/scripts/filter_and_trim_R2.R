...
report_progress <- function(counts) {
    ...
    with(as.list(counts), {
        if(total_counts %% iteration_gap == 0L) {
...
summarize_results <- function(counts) {
    ...
    message(glue("of which {n_kept_reads} reads ({n_kept_reads/total_reads*100}%) were selected due from barcode frequency"))
    ...
    message(glue("A total of {kept_reads} ({kept_reads/n_kept_reads*100}% of the selected reads) were written to the output FASTQ file"))
    }
    )
}

# Initializes all counts to zero by calling the filtering chain on an empty input stringset
counts <- processing_chain(character() |> DNAStringSet())
