...
trim_bc1 <- function(chunk) {
    ...
    map_int("trim_count") |>
    sum()
    trimmed_chunk <- rlang::exec(c, !!!unname(bc1_trimmed_R2_list) |> map("sequences"))
...
trim_hairpins <- function(chunk) {
    ...
    filtered_chunk <- rlang::exec(c, !!!unname(hairpin_filter) |> map("sequences"))
...
filter_frequency <- function(chunk) {
    ...
    list(chunk=chunk[keep_idx], counts = c(total_reads=total_reads, kept_reads = n_kept_reads))
}
