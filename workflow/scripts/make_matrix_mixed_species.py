...
sample = sys.argv[1]
table = pd.read_csv(f"results/{sample}/{sample}_filtered_mapped_UMIs.txt",sep='\t',index_col=0)
...
gene_matrix.to_csv(f"results/{sample}/{sample}_gene_count_matrix.txt",sep='\t')
