...
cells.2 = names(subset(seurat,idents='1')$cell.type)

ids = c('tet1_20211014','tet5_20211014','tet1hr_20230919')
cells.1 = names(subset(seurat,subset=(cell.type %in% ids))$cell.type)
save_markers(seurat,seurat_full,cells.1,cells.2,'source_data/generated/v4_tet_vs_1.txt')
...
save_markers(seurat,seurat_full,cells.1,cells.2,'source_data/generated/v4_metG_clust2_vs_1.txt')

ids = levels(seurat$cell.type)[(grepl('d6_1hr',levels(seurat$cell.type)))]
cells.1 = names(subset(seurat,subset=(cell.type %in% ids) & (seurat_clusters=='2'))$cell.type)
save_markers(seurat,seurat_full,cells.1,cells.2,'source_data/generated/v4_d6_clust2_vs_1.txt')

ids = levels(seurat$cell.type)[(grepl('h1|h2|h3',levels(seurat$cell.type))) & !(grepl('10min|20min',levels(seurat$cell.type)))]
cells.1 = names(subset(seurat,subset=(cell.type %in% ids) & (seurat_clusters=='2'))$cell.type)
save_markers(seurat,seurat_full,cells.1,cells.2,'source_data/generated/v4_hipA7_clust2_vs_1.txt')
